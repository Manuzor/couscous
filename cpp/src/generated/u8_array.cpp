...
Reserve(u8_array* Array, int RequiredCapacity)
{
    ...
    if (RequiredCapacity > Array->Capacity)
    {
        ...
        if (Array->_Data)
        {
            ::mtb::CopyBytes(NewData, Array->_Data, Array->NumElements * sizeof(u8));
            free(Array->_Data);
        }
        else
        {
            ::mtb::CopyBytes(NewData, Array->_Fixed, Array->NumElements * sizeof(u8));
        }
...
RemoveRange(u8_array* Array, int FirstIndex, int OnePastLastIndex)
{
    ...
    if(OnePastLastIndex > FirstIndex && IsValidIndex(Array, FirstIndex) && IsValidIndex(Array, OnePastLastIndex - 1))
    {
        ...
        ::mtb::CopyBytes(Array->Data() + FirstIndex, Array->Data() + OnePastLastIndex, NumTrailing * sizeof(u8));
