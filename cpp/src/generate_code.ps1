...
"@}
    ...
    if (RequiredCapacity > Array->Capacity)
    {
        ...
        if (Array->_Data)
        {
            ::mtb::CopyBytes(NewData, Array->_Data, Array->NumElements * sizeof($($Array.Type)));
            free(Array->_Data);
        }
$(if($HasFixed)
{@"
        ...
        {
            ::mtb::CopyBytes(NewData, Array->_Fixed, Array->NumElements * sizeof($($Array.Type)));
        }
"@}
)
...
RemoveRange($($Array.Name)* Array, int FirstIndex, int OnePastLastIndex)
{
    ...
    if(OnePastLastIndex > FirstIndex && IsValidIndex(Array, FirstIndex) && IsValidIndex(Array, OnePastLastIndex - 1))
    {
        ...
        ::mtb::CopyBytes(Array->Data() + FirstIndex, Array->Data() + OnePastLastIndex, NumTrailing * sizeof($($Array.Type)));
...
${CommonFuncPrefix}strc
StrConst($($Text.Name) Text)
{
